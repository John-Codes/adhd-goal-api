...
class GoalMonth:
    """A simple 30-day rolling goal tracker with CRUD operations and persistence."""
    
    def __init__(self, storage_file: str = "monthly_goals_data.txt"):
        self.storage_file = storage_file
        self.goals: List[Dict[str, Any]] = []
        self.load_goals()
    
    def create(self, name: str, description: str, due_date: Optional[str] = None) -> int:
        """Create a new 30-day rolling goal with auto-assigned ID."""
        goal_data = {
            "id": len(self.goals),
            "name": name,
    ...
    def update(self, goal_id: int, name: Optional[str] = None, description: Optional[str] = None, 
               due_date: Optional[str] = None, is_active: Optional[bool] = None) -> bool:
        ...
        if due_date is not None:
            self.goals[goal_id]["due_date"] = due_date
    ...
    def get_expired_goals(self) -> List[Dict[str, Any]]:
        """Get all expired goals (30+ days old)."""
        expired = []
        for goal in self.goals:
            if goal["is_active"]:
                goal_info = self.read(goal["id"])
...
def main():
    """Test the 30-day rolling goal tracker."""
    print("=== 30-Day Rolling Goal Tracker ===\n")
    ...
    goal1_id = tracker.create("Fitness", "Run 3x per week", "2025-12-01")
    goal2_id = tracker.create("Reading", "Read 2 books this month", "2025-12-15")
    goal3_id = tracker.create("Project", "Complete side project MVP", "2025-11-30")
    ...
    tracker.update(goal1_id, name="Fitness Goal", description="Run 4x per week", due_date="2025-12-10")
