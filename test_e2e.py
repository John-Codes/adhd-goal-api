#!/usr/bin/env python3
"""
Comprehensive E2E tests for Goals API and LLM endpoint with optional API key support
"""
import requests
import json
import os
...
def test_health_check():
    ...
    print(f"Using API key from env: {REAL_API_KEY[:20]}...")
    try:
...
def test_llm_without_api_key():
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/api/llmquerygoals", json=request_data)
        print(f"Status: {response.status_code}")
...
def test_llm_invalid_model():
    """Test LLM endpoint with invalid model"""
    print("10. Testing LLM endpoint with invalid model...")
    try:
        request_data = {
            "message": "Hello there!",
            "model": "nonexistent/model:1.0"
        }
        
        response = requests.post(f"{BASE_URL}/api/llmquerygoals", json=request_data)
        print(f"Status: {response.status_code}")
        
        # This might fail due to model not existing, which is expected
        if response.status_code == 200:
            print("PASS: Invalid model handled gracefully")
        elif response.status_code == 500:
            result = response.json()
            if "model" in result.get("detail", "").lower():
                print("PASS: Model validation working correctly")
            else:
                print("FAIL: Unexpected error")
                return False
        else:
            print(f"UNEXPECTED: {response.status_code} - {response.json()}")
...
def main():
    """Run all E2E tests"""
    print("Comprehensive E2E Testing for Goals API and LLM Endpoint")
    print("=" * 70)
    ...
    if tests_passed == total_tests:
        print("SUCCESS: All tests passed! The API is working correctly with optional API key support.")
    else:
